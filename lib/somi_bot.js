...
var SOMI_API_DOMAIN = 'http://95.172.148.149/api/v1/';
var SOMI_DRUPAL_DOMAIN = 'http://95.172.148.149/';
                  ...
                  if (semaphore) {
                    ...
                    if (item && item.data && item.data.slack_id) {
                      ...
                      if (owner) {
                        ...
                        self._sendOwnerPrivateMessage(owner, 'remind', message, item);
                      }
                    }
...
SomiBot.prototype._submitCrystalOperation = function (message) {
  ...
  if (response.error) throw {code: 1, message: response.error};
}
  ...
  if (action == 'remind') {
    ...
    if (message.user) {
      ...
      if (user = this._getUserById(message.user)) {
        ...
        author_link = SOMI_DRUPAL_DOMAIN + (typeof this.api_initiator ? ('admin/user/' + this.api_initiator.uid + '/account') : '';
    ...
    switch(type) {
      ...
      case 'channel':
        if (action == 'grant') {
          ...
          block_header = '*Сделка на %qty% ' + plural_str(this.quantity, 'кристалл', 'кристалла', 'кристаллов') + ' прошла успешно!* :white_check_mark:\n\n*От кого*';
          block_header = block_header.replace(/%qty%/g, this.quantity);
...
SomiBot.prototype._sendOwnerPrivateMessage = function (owner, action, message, item) {
  // To initiator.
  var ims = this._getImsById(message.user);
  if (ims) {
    var type = 'owner_private';
