...
var SOMI_CURRENCIES_TRANSLATIONS = {
  gem: {
    1: 'Кристалл', 2: 'Кристалла', 3: 'Кристаллов', 4: 'Кристаллами', 5: 'Кристалл', 6: 'Кристалл', 7: 'Кристалла', 8: 'Кристаллов',
    op: 'Награждение', op2: 'Награждении', op3: 'Награжден', op4: 'Награждены', op5: 'Подарить', op6: 'Вам подарили', op7: 'Вы подарили',
    opColor : 'good'
  },
  beetle: {
    1: 'Жук', 2: 'Жука', 3: 'Жуков', 4: 'Жуками', 5: 'Жуком', 6: 'Жука', 7: 'Жуками', 8: 'Жуками',
    op: 'Наказание', op2: 'Наказании', op3: 'Наказан', op4: 'Наказаны', op5: 'Наказать', op6: 'Вы получили в наказание ', op7: 'Вы наказали',
    opColor : 'danger'
  }
};
...
SomiBot.prototype._onMessage = function (message) {
  ...
  if (message.type == 'message') {
    ...
    if (!this._isFromSomiBot(message)) {
      ...
      var currency, currency_regexp, currency_key;
              ...
              for (var i in transactions) {
                if (transactions.hasOwnProperty(i)) {
                  ...
                  if (currencies.length) {
                    ...
                    try {
                      if (this.quantity && (typeof this.quantity == 'string') && this.quantity.match(/\.[1-9]/)) {
                        throw {code: 3, message: "Операции с дробными числами запрещены."}
                      }
                      else if (this.quantity.length > 5) {
                        ...
                        if (semaphore) {
                          ...
                          if (item && item.data && item.data.slack_id) {
                            ...
                            if (owner) {
                              ...
                              this.currency_key = item.data.currency.replace(/:/g, '');
...
SomiBot.prototype._prepareMessageOptions = function (action, type, message, post_api_user) {
  ...
  recipient = '';
  header = SOMI_CURRENCIES_TRANSLATIONS[this.currency_key].op + ' ' + SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][4].toLowerCase();
  event = 'undefined';
  block_header = 'undefined';
  color = SOMI_CURRENCIES_TRANSLATIONS[this.currency_key].opColor;
  thumb_url = '';
  author_icon = '';
  ...
  if (action == 'remind') {
    color = 'warning';
    event = '_Уведомление_';
    block_header = ':bell: *Вы хотели подарить %qty%* ' + this.currency;
    block_header = block_header.replace(/%qty%/g, message.attempt_transaction_amount);
    ...
    for (var key in recipients) {
      if (recipients.hasOwnProperty(key)) {
        ...
        for (var i = 0; i < message.transaction_recipient_quantity; ++i) {
    ...
    goods = plural_str(
      this.quantity,
      SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][1].toLowerCase(),
      SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][2].toLowerCase(),
      SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][3].toLowerCase()
    );

    if (action == 'error') {
      var total_goods = plural_str(
        this.total_transaction_op_quantity,
        SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][1].toLowerCase(),
        SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][2].toLowerCase(),
        SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][3].toLowerCase()
      ...
      block_header = '*Сделка на %qty% ' + total_goods + ' не удалась!* :no_entry_sign:';
      block_header = block_header.replace(/%qty%/g, this.total_transaction_op_quantity);
      block_header += '\n\n*Отправитель*';

      event = '_Произошла ошибка при ' + SOMI_CURRENCIES_TRANSLATIONS[this.currency_key].op2.toLowerCase() + '_';
      color = 'danger';
    ...
    switch(type) {
      case 'private':
        if (action == 'grant') {
          var received_goods = plural_str(
            this.quantity,
            SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][6].toLowerCase(), // жуком
            SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][7].toLowerCase(), // жуками
            SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][3].toLowerCase()
          );

          block_header = '*Вы получили %qty% ' + goods + '!*';
          for (var i = 0; i < this.quantity; ++i) {
            block_header += ' ' + this.currency;
          }
          block_header += '\n\n*От кого*';

          block_header = block_header.replace(/%qty%/g, this.quantity);

          event = '_' + SOMI_CURRENCIES_TRANSLATIONS[this.currency_key].op6  + ' %qty% ' + received_goods +  ' от %usr%_';
      ...
      case 'owner_private':
        if (action == 'grant') {
          var spent_goods = plural_str(
            this.quantity,
            SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][5].toLowerCase(), // жуком
            SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][7].toLowerCase(), // жуками
            SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][4].toLowerCase()
          );

          event = '_' + SOMI_CURRENCIES_TRANSLATIONS[this.currency_key].op7 + ' %qty% ' + spent_goods + ' %users%_';
          event = event.replace(/%qty%/g, this.total_transaction_op_quantity);
      ...
      case 'channel':
        if (action == 'grant') {
          event = plural_str(this.recipientsCount, '_Был(а) '
            + SOMI_CURRENCIES_TRANSLATIONS[this.currency_key].op3.toLowerCase() + '(а) %users%_',
            '_Были ' + SOMI_CURRENCIES_TRANSLATIONS[this.currency_key].op4.toLowerCase() + ' %users%_',
            '_Были ' + SOMI_CURRENCIES_TRANSLATIONS[this.currency_key].op4.toLowerCase() + ' %users%_');
          ...
          block_header = '*Сделка на %qty% ' + total_goods + ' прошла успешно!* :white_check_mark:\n\n*От кого*';
          block_header = block_header.replace(/%qty%/g, this.total_transaction_op_quantity);
  ...
  if (goods_remain !== false) {
    this.options.attachments[0].fields.push({
      title: SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][3] + ' осталось',
  ...
  if (goods_granted !== false) {
    var title = action == 'remind'
      ? SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][3]+ ' в наличии'
      : SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][3] + ' на вашем счете';
  ...
  if (goods_needed !== false) {
    this.options.attachments[0].fields.push({
      title: 'Не хватает ' + SOMI_CURRENCIES_TRANSLATIONS[this.currency_key][3].toLowerCase(),
  ...
  if (action == 'remind') {
    this.options.attachments[0].fields.push(  {
      ...
      value: ':arrows_counterclockwise: <slack-action://BCRYSTALBOT/repeat/crystal-op/' + message.item_id + '|' +
        SOMI_CURRENCIES_TRANSLATIONS[this.currency_key].op5.toLowerCase() + '>',
      short: short_format
    })
  }
}
