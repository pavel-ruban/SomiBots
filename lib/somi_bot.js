...
SomiBot.prototype._prepareMessageOptions = function (action, type, message, post_api_user) {
  ...
  if (action == 'remind') {
    ...
    if (post_api_user.slack_id) {
      var user;

      if (user = this._getUserById(post_api_user.slack_id)) {
        author_name = user.name;
        author_link = SOMI_DRUPAL_DOMAIN + (post_api_user.uid ? ('admin/user/' + post_api_user.uid + '/account') : '');
        author_icon = user.profile.image_32;
        thumb_url = user.profile.image_72;
      }
      else {
        author_link = SOMI_DRUPAL_DOMAIN + (post_api_user && post_api_user.uid ? ('admin/user/' + post_api_user.uid + '/account') : '');
      }
    }
  }
  else {
