        ...
        case Boolean(this.currencies.length):
          for (var i in transactions) {
            if (transactions.hasOwnProperty(i)) {
              var currencies = getMatches(transactions[i], /(:gem:)/g, 1);
              ...
              if (currencies.length) {
                ...
                try {
                  if (this.quantity.match(/\.[1-9]/)) {
                    throw {code: 3, message: "Операция с дробными числами запрещены."}
                  }
...
SomiBot.prototype._getCrystalAmount = function (message) {
  var quantifier = message.text.match(/(-?\b[0-9]+?(?:\.[0-9]+)?)\b[\s\t]*:gem:/, 1);
