...
var SOMI_CURRENCIES_TRANSLATIONS = {
  gem: {
    1: 'Кристалл', 2: 'Кристалла', 3: 'Кристаллов', 4: 'Кристаллами',
    op: 'Награждение', op2: 'Награждении', op3: 'Награжден', op4: 'Награждены', op5: 'Подарить', opColor : 'good'
  },
  beetle: {
    1: 'Жук', 2: 'Жука', 3: 'Жуков', 4: 'Жуками',
    op: 'Наказание', op2: 'Наказании', op3: 'Наказан', op4: 'Наказаны', op5: 'Наказать', opColor : 'danger'
  }
};
    ...
    if (!this._isFromSomiBot(message)) {
      ...
      for (var c in SOMI_CURRENCIES) {
        if (SOMI_CURRENCIES.hasOwnProperty(c)) {
          ...
          this.currencies = getMatches(message.text, this.currency_regexp, 1);
          if (this.currencies.length && !transactions) {
            transactions = message.text.match(/(.+)(?:\n|$)/g);
          }
