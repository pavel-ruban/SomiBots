...
SomiBot.prototype._getThumbUrl = function() {
  ...
  // Comment it yet. This logic should build dynamic images from 4 images.
  if (image_urls.length > 1 && image_urls.length < 5) {
    var gm = require('gm');
    ...
    for (var i in image_urls) {
      if (image_urls.hasOwnProperty(i)) {
        var file = fs.createWriteStream("thump_urls/" + image_urls[i]);
        var request = http.get(image_urls[i].replace('https', 'http'), function (response) {
          response.pipe(file);
        });
      }
    }
    ...
    gm()
      ...
      .write('/hosts/somi_d8/slack-tumb-url.jpg', function (err) {
        if (err) console.log(err);
      });

      return SOMI_DRUPAL_DOMAIN + '/slack-tumb-url.jpg';
  }
  else if (image_urls.length && image_urls[0])
  {
    return image_urls[0];
  }
