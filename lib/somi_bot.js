...
var SOMI_CURRENCIES_TRANSLATIONS = {
  ...
  drupal: {
    ...
    event_remind: '_Напоминание_',
    event_private: {msg: '_Вас отдрупалил(а) %qty% %goods% %usr%_', case: 'instrumental'},
    event_owner: {msg: '_Вы отдрупалили %qty% %goods% %users%_', case: 'instrumental'},
    event_channel: {alt: '_Был(а) отдрупален(а) %users%_', msg: '_Были отдрупалены %users%_', case: 'nominative'},

    op_status: {msg: '*Друпликация на %qty% %goods% прошла успешно!* :white_check_mark:\n\n*От кого*', case: 'nominative'},
    op_status_private: {msg: '*Вас отдрупалили %qty% %goods%!* %currencies%\n\n*От кого*', case: 'instrumental'},
    op_status_remind: {msg: ':bell: *Вы хотели отдрупалить %qty% %currency%*', case: 'instrumental'},
