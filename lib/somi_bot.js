    ...
    if (!this._isFromSomiBot(message)) {
            ...
            case Boolean(this.currencies.length):
              for (var i in transactions) {
                if (transactions.hasOwnProperty(i)) {
                  // Ignore qoute messages.
                  if (transactions[i].match(/^\s*\t*&gt;/)) continue;
      ...
      for (var war in SOMI_NOTIFICATIONS) {
        ...
        for (var key in SOMI_NOTIFICATIONS[war].react_on) {
          ...
          // Nasty js doesn't have look behind to let us be able to check different cases we have to split single regexp
          // rule to several ones.
          warning_regexp = new RegExp(warning + '(?!\.biz|\.org|\.com)(?:.|\n)*$', 'i');
          // Check message for warnings within.
          if (warning_regexp.test(origin_message)) {

            warning_regexp = new RegExp(warning + '(?!\.biz|\.org|\.com)(?:.|\n)*$', 'ig');
            var warning_with_link_regexp = new RegExp(':\/\/' + warning, 'ig');

            var wrong_count = (origin_message.match(warning_with_link_regexp) || []).length,
              correct_count = (origin_message.match(warning_regexp) || []).length;

            if (wrong_count < correct_count && !processed) {
