  ...
  if (message.type == 'message') {
    ...
    if (!this._isFromSomiBot(message)) {
      ...
      if (SOMI_API_BEARER) {
        ...
        if (!res.error &&  !res.status && Object.keys(res).length && res.timestamp && res.config) {
      ...
      // Send channel messages.
      for (var war in res.warnings) {
        this.color = res.warnings[war].color;
        this.op_header = res.warnings[war].op_header;
        this.icon = res.warnings[war].icon;
        // Retrieve warning context.
        warning = res.warnings[war].react_on;
        warning_regexp = new RegExp('(' + warning + ')', 'g');

        // Check message for warnings within.
        this.warning = getMatches(origin_message, warning_regexp, 1);
        if (this.warning.length && !transactions) {
          transactions = origin_message.match(/(.+)(?:\n|$)/g);
          this.message = message;

          message.text = transactions[0];
          this.action = 'warning';
          self._sendChannelMessages(this.action, message);

          var res = request('POST', SOMI_API_DOMAIN + 'create/history', {
            ...
            json: {
              ...
              tid: war,
              webhooks: res.warnings[war].webhooks
            }
          });
        }
      }
